...
If you want to use `@fastify/multipart` with `@fastify/swagger` and `@fastify/swagger-ui` you must add a new type called `isFile` and use custom instance of validator compiler [Docs](https://www.fastify.dev/docs/latest/Reference/Validation-and-Serialization/#validator-compiler).
