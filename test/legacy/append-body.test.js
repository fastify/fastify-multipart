...
test('addToBody with serialized object', t => {
  ...
  const opts = {
    addToBody: true,
    enableSpecialNotationKeys: true
  }
  fastify.register(multipart, opts)
...
test('addToBody with serialized object and parseArrays=false', t => {
  ...
  const opts = {
    addToBody: true,
    enableSpecialNotationKeys: 'objectsOnly'
  }
  fastify.register(multipart, opts)
...
const deserializationErrorTestCases = [
  ...
  { description: 'a key with "constructor"', sampleKeys: ['foo[][bar][constructor]', 'constructor[0]', 'constructor[foo]', 'constructor{}'], sampleValue: '[]', expectedError: 'constructor is not allowed as a key name' },
  { description: 'a key with "__proto__"', sampleKeys: ['foo[0][__proto__]', '__proto__[]', '__proto__[foo][bar][]', 'foo[__proto__]'], expectedError: '__proto__ is not allowed as a key name' },
  ...
  { description: 'a key with the "{}" special ending whose value contains __proto__ as key', sampleKeys: ['foo{}'], sampleValue: '{"__proto__": 1}', expectedError: 'the {} special ending was used, but value is not a valid JSON' }
]
...
test('addToBody with serialized object and enableSpecialNotationKeys=false', t => {
  t.plan(4)

  const fastify = Fastify()
  t.teardown(fastify.close.bind(fastify))

  // The enableSpecialNotationKeys option is false by default, so we don't need to set it
  const opts = {
    addToBody: true
  }
  fastify.register(multipart, opts)
  ...
  fastify.post('/', function (req, reply) {
    t.strictSame(req.body, {
      'propA[propAA]': '11',
      'propA[propAB]': '22',
      'propA[propAC][]': ['111', '222', '333'],
      'propB[0][propBA][propBAA]': 'foo',
      'propB[0][propBA][propBAB]': 'zoo',
      'propB[0][propBA][propBAC][0]': '777',
      'propB[0][propBA][propBAC][1]': '888',
      'propB[0][propBA][propBAC][2]': '999',
      'propB[0][propBB]': 'zaz',
      'propB[1][propBA][propBAA]': 'bar',
      'propC[]': ['1', '2', '3'],
      propD: 'baz',
      propE: ['4', '5', '6'],
      'propF{}': '{"foo":[{"bar":[111,222,333]}]}',
      'propG[0][propGA][]{}': '{"zaz":[{"zoo":"baz"}]}',
      'propG[1]{}': '[11,22,{"foo":"bar"}]'
    })
